...
/*
    Инкапсуляция — это принцип, при котором данные и методы, оперирующие 
    этими данными, объединяются в единый объект. Доступ к данным осуществляется 
    через публичные методы(интерфейсы), что позволяет скрывать внутреннюю 
    реализацию и предотвращает прямое вмешательство в состояние объекта, 
    обеспечивая его целостность и безопасность.

    Наследование — это механизм, позволяющий создавать новые классы 
    на основе уже существующих, перенимая их свойства и методы.
    Это способствует повторному использованию кода, упрощению 
    его расширения и поддержке, а также созданию иерархий классов, 
    отражающих логические связи между ними.

    Полиморфизм — это способность объектов разных типов обрабатывать 
    данные через единый интерфейс.Полиморфизм позволяет 
    использовать один и тот же метод для объектов разных классов, 
    обеспечивая динамическое переопределение поведения и адаптацию программы к
    работе с разными типами данных.
*/
...
// Шаблонная функция может не быть полиморфной, если шаблонный тип не
// является частью сигнатуры.
...
/*
    ...
    Ассоциация в объектно-ориентированном программировании представляет
    собой связь между двумя классами, когда один класс использует другой, но
    при этом ни один из них не владеет другим.
